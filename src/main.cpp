...
FILE_TYPE get_extension(std::string file)
{
    ...
    {
        if (fs::path(file).stem().extension() == ".nii")
        {
            return F_NII;
        }
    }
    else if (fs::path(file).extension() == ".nii")
    {
        return F_NII;
    }
    else if (fs::path(file).extension() == ".vmr")
    {
        return F_VMR;
    }
    else if (fs::path(file).extension() == ".v16")
    {
        return F_V16;
    }
    else if (fs::path(file).extension() == ".fmr")
    {
        return F_FMR;
    }
    else if (fs::path(file).extension() == ".vtc")
    {
        return F_VTC;
    }
    else
...
int main(int argc, char **argv)
{
    ...
    if (current_config.file_in_type == F_VMR || current_config.file_out_type == F_VMR)
        vmr2nii(current_config);
    if (current_config.file_in_type == F_FMR || current_config.file_out_type == F_FMR)
        fmr2nii(current_config);
